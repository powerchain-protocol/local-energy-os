...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!organizationId && "charging" !== "admin" && "charging" !== "mapper" && "charging" !== "grid") {
    ...
    setLoading(true);
    setError(null);
    client.get<ApiData>("/api/v1/charging/stations", controller.signal)
  ...
  return <main className="pc-page">
    <PageHeader
      ...
      title="EV Charging"
      description="OCPP and ISO 15118 aligned charging infrastructure with V2G-ready operating boundaries."
      action={<StatusBadge tone={error ? "warning" : "success"}>{loading ? "Loading" : error ? "Unavailable" : "API connected"}</StatusBadge>}
    />
    <div className="pc-grid">
      <div className="pc-span-3"><StatCard label="Charging stations" value={count ?? (data ? fieldCount : "—")} meta={count === null ? "Canonical projection fields" : "Records in current scope"} /></div>
