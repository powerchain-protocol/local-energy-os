...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!organizationId && "supply-chain" !== "admin" && "supply-chain" !== "mapper" && "supply-chain" !== "grid") {
    ...
    setLoading(true);
    setError(null);
    client.get<ApiData>("/api/v1/supply-chain/passports", controller.signal)
  ...
  return <main className="pc-page">
    <PageHeader
      eyebrow="Infrastructure Provenance"
      title="Supply Chain"
      description="Energy-infrastructure asset passports, custody history and provenance records."
      action={<StatusBadge tone={error ? "warning" : "success"}>{loading ? "Loading" : error ? "Unavailable" : "API connected"}</StatusBadge>}
    />
    <div className="pc-grid">
      <div className="pc-span-3"><StatCard label="Asset passports" value={count ?? (data ? fieldCount : "—")} meta={count === null ? "Canonical projection fields" : "Records in current scope"} /></div>
      ...
      <Panel className="pc-span-8" eyebrow="Operations" title="Canonical service boundary" action={<code className="pc-code">/api/v1/supply-chain/passports</code>}>
        {error ? <EmptyState icon="supply" title="Data source unavailable" description={error} /> : loading ? <EmptyState icon="status" title="Loading operational state" description="The workspace is resolving tenant-scoped data from the canonical API." /> : <div className="pc-data-list"><div className="pc-data-row"><div><strong>Physical / operational source</strong><p>Supply-chain evidence is committed without turning every event into blockchain state.</p></div><span>Canonical</span></div><div className="pc-data-row"><div><strong>Tenant isolation</strong><p>Organization context is forwarded to API policy and repository boundaries.</p></div><span>{organizationId ? "Scoped" : "Unconfigured"}</span></div><div className="pc-data-row"><div><strong>Economic state</strong><p>Blockchain and settlement details remain secondary until an operation requires them.</p></div><span>Separated</span></div></div>}
      </Panel>
