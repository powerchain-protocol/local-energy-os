...
export interface HeliumAdapterRequest {
  operation: string;
  payload?: Record<string, unknown>;
}
export interface HeliumAdapterResponse {
  provider: "helium";
  operation: string;
  payload: Record<string, unknown>;
}
...
export class HeliumAdapter implements IntegrationAdapter<
  HeliumAdapterRequest,
  HeliumAdapterResponse
> {
  ...
  async execute(
    request: HeliumAdapterRequest,
    context: IntegrationContext,
  ): Promise<IntegrationResult<HeliumAdapterResponse>> {
    if (!this.endpoint)
      return {
        state: "misconfigured",
        ...
        error: {
          ...
          message: "Helium endpoint is not configured",
          retryable: false,
        },
      };
    ...
    try {
      ...
      return {
        ...
        source: this.provider,
        observedAt: new Date().toISOString(),
        data: { provider: "helium", operation: request.operation, payload },
      };
    } catch (error) {
      return {
