...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!organizationId && "companies" !== "admin" && "companies" !== "mapper" && "companies" !== "grid") {
    ...
    setLoading(true);
    setError(null);
    client.get<ApiData>("/api/v1/participants/summary", controller.signal)
  ...
  return <main className="pc-page">
    <PageHeader
      ...
      description="Company and client operating workspace backed by tenant-scoped participant and energy services."
      action={<StatusBadge tone={error ? "warning" : "success"}>{loading ? "Loading" : error ? "Unavailable" : "API connected"}</StatusBadge>}
    />
    <div className="pc-grid">
      <div className="pc-span-3"><StatCard label="Participant projection" value={count ?? (data ? fieldCount : "—")} meta={count === null ? "Canonical projection fields" : "Records in current scope"} /></div>
