...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!organizationId && "wind" !== "admin" && "wind" !== "mapper" && "wind" !== "grid") {
    ...
    setLoading(true);
    setError(null);
    client.get<ApiData>("/api/v1/wind-farms", controller.signal)
  ...
  return <main className="pc-page">
    <PageHeader
      ...
      title="Wind Farms"
      description="Wind-farm portfolios, turbine infrastructure and verified generation boundaries."
      action={<StatusBadge tone={error ? "warning" : "success"}>{loading ? "Loading" : error ? "Unavailable" : "API connected"}</StatusBadge>}
    />
    <div className="pc-grid">
      <div className="pc-span-3"><StatCard label="Wind farms" value={count ?? (data ? fieldCount : "—")} meta={count === null ? "Canonical projection fields" : "Records in current scope"} /></div>
      ...
      <Panel className="pc-span-8" eyebrow="Operations" title="Canonical service boundary" action={<code className="pc-code">/api/v1/wind-farms</code>}>
        {error ? <EmptyState icon="wind" title="Data source unavailable" description={error} /> : loading ? <EmptyState icon="status" title="Loading operational state" description="The workspace is resolving tenant-scoped data from the canonical API." /> : <div className="pc-data-list"><div className="pc-data-row"><div><strong>Physical / operational source</strong><p>MWh Energy RWA remains subordinate to verified physical production.</p></div><span>Canonical</span></div><div className="pc-data-row"><div><strong>Tenant isolation</strong><p>Organization context is forwarded to API policy and repository boundaries.</p></div><span>{organizationId ? "Scoped" : "Unconfigured"}</span></div><div className="pc-data-row"><div><strong>Economic state</strong><p>Blockchain and settlement details remain secondary until an operation requires them.</p></div><span>Separated</span></div></div>}
      </Panel>
