...
export interface JupiterAdapterRequest {
  operation: string;
  payload?: Record<string, unknown>;
}
export interface JupiterAdapterResponse {
  provider: "jupiter";
  operation: string;
  payload: Record<string, unknown>;
}
...
export class JupiterAdapter implements IntegrationAdapter<
  JupiterAdapterRequest,
  JupiterAdapterResponse
> {
  ...
  async execute(
    request: JupiterAdapterRequest,
    context: IntegrationContext,
  ): Promise<IntegrationResult<JupiterAdapterResponse>> {
    if (!this.endpoint)
      return {
        state: "misconfigured",
        ...
        error: {
          ...
          message: "Jupiter endpoint is not configured",
          retryable: false,
        },
      };
    ...
    try {
      ...
      if (!response.ok)
        return {
          ...
          error: {
            ...
            message: "Jupiter request failed",
      ...
      return {
        ...
        source: this.provider,
        observedAt: new Date().toISOString(),
        data: { provider: "jupiter", operation: request.operation, payload },
      };
    } catch (error) {
      return {
