...
const { root, environment, effectiveUrl } = resolveDatabaseEnvironment();
...
const target = parseDatabaseTarget(effectiveUrl);
if (!target) failNoUrl();

async function doctor(requireReachable = true) {
  console.log(`[PowerChain DB] target ${target.host}:${target.port}/${target.database}${isLocalDatabaseHost(target.host) ? " (local)" : " (managed)"}`);
...
switch (action) {
  ...
  case "wait":
    ...
    break;
  case "down":
    if (!isLocalDatabaseHost(target.host)) { console.log("[PowerChain DB] Managed datasource configured; nothing to stop locally."); break; }
  ...
  case "setup": {
    ...
    if (isLocalDatabaseHost(target.host) && !await connect(target.host, target.port)) {
      const result = run("node", ["tools/infra.mjs", "up"]);
      if (result.status !== 0) process.exit(result.status ?? 1);
