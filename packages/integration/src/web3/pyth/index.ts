...
export interface PythAdapterRequest {
  operation: string;
  payload?: Record<string, unknown>;
}
export interface PythAdapterResponse {
  provider: "pyth";
  operation: string;
  payload: Record<string, unknown>;
}
...
> {
  ...
  async execute(
    request: PythAdapterRequest,
    context: IntegrationContext,
  ): Promise<IntegrationResult<PythAdapterResponse>> {
    if (!this.endpoint)
      return {
        state: "misconfigured",
        ...
        error: {
          ...
          message: "Pyth Hermes endpoint is not configured",
          retryable: false,
        },
      };
    ...
    try {
      ...
      if (!response.ok)
        return {
          ...
          error: {
            ...
            message: "Pyth Hermes request failed",
      ...
      return {
        ...
        source: this.provider,
        observedAt: new Date().toISOString(),
        data: { provider: "pyth", operation: request.operation, payload },
      };
    } catch (error) {
      return {
